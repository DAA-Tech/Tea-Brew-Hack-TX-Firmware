...
//SHT11 connections
extern sfr sbit SDA_pin_out;          // Serial data out pin
...
{
  //Initial state
  SDA_pin_out = 1;                       //release DATA-line
  SCL_pin = 0;                           // SCL Low
...
unsigned char s_write_byte(unsigned char value)
{
  ...
  { //shift bit for masking
    if (i & value)
    {
      SDA_pin_out = 1;    //masking value with i , write to SENSI-BUS
    }
    else
    {
      SDA_pin_out = 0;
  ...
  SCL_pin = 1;            //clk #9 for ack
  Delay_uS(3);
  if (SDA_pin_in == 1)  error = 1; //check ack (DATA will be pulled down by SHT11)
  Delay_uS(1);
  SCL_pin = 0;
